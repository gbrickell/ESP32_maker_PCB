// microwave_detection.ino ESP32 kits PCB simple microwave object detection using the RCWL-0516 sensor
//   **** NB this uses the standard 'PIR' input pins by inserting and only using the 3 inner most pins
//         and powers the unit at 5V and the signal pin is #36
//
...
int trigger_interval = 3000; // minimum interval in milliseconds between detections to allow the 
                             // electronics to reset - the usual default on the electronics is 3 seconds
int counter = 0;
...
void setup() {
  ...
  Serial.println(" RCWL-0516 microwave detection program starting: ");
...
void loop() {
  ...
  if (Current_State == 1) {
    ...
    counter = counter + 1;
    Serial.print (" RCWL-0516 detected moving body: " );
    ...
    Serial.println(" .... now looking again for a moving body to be detected");
    Current_State  = 0;
  }
