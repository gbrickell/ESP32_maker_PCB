// 80x160_ST7735_drawXBitmap.ino  SPI display example for the ESP32 Maker Kit
...
#include "cat107x80.h"       // Sketch tab header for the cat image
#include "star80x80.h"       // Sketch tab header for the star image
...
void loop() {
  ...
  tft.drawXBitmap(40, 0, star, starWidth, starHeight, TFT_BLACK);
  delay(2000);
  tft.fillScreen(TFT_BLACK); // Black screen fill
  ...
  // Bits set to 1 plot as the defined color, bits set to 0 are not plotted
  //               x  y  xbm  xbm width xbm height color
  tft.drawXBitmap(26, 0, cat, catWidth, catHeight, TFT_BLACK);
  delay(2000);
  tft.fillScreen(TFT_BLACK); // Black screen fill
