/// red_amber_green_LED_flash.ino ESP32 maker kit version of LED_red_amber_green_flash.py
...
void loop() {
  ...
  if (printon) {
    Serial.println("blink the red, amber and green LEDs on/off with a 0.5 second delay");
    printon = false;
  }
  
  digitalWrite(red_positive_pin, HIGH);
  digitalWrite(amber_positive_pin, LOW);
  digitalWrite(green_positive_pin, LOW);
  delay(500);
  digitalWrite(red_positive_pin, LOW);
  digitalWrite(amber_positive_pin, HIGH);
  digitalWrite(green_positive_pin, LOW);
  delay(500);
  digitalWrite(red_positive_pin, LOW);
  digitalWrite(amber_positive_pin, LOW);
